...
export default function TicketPage() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4 flex items-start justify-center" style={{ background: 'var(--bg-base)' }}>
      <div className="w-full max-w-md">
        <Link to="/my-tickets" className="flex items-center gap-2 text-white/40 hover:text-white text-sm mb-6 transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <p className="text-white/40 text-xs font-bold uppercase tracking-widest mb-1">Événement</p>
                <h2 className="text-white font-bold text-xl leading-tight">{ticket.event_title}</h2>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                {date && (
                  <>
                    <div>
                      <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1"><Calendar size={11} /> Date</div>
                      <p className="text-white font-semibold text-xs">{date.toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'long' })}</p>
                    </div>
                    <div>
                      <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1"><Clock size={11} /> Heure</div>
                      <p className="text-white font-semibold text-xs">{date.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                  </>
                )}
                <div>
                  <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1"><MapPin size={11} /> Lieu</div>
                  <p className="text-white font-semibold text-xs">{ticket.event_location}</p>
                </div>
                <div>
                  <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1"><User size={11} /> Participant</div>
                  <p className="text-white font-semibold text-xs">{ticket.holder_name}</p>
                </div>
                {ticket.holder_phone && (
                  <div>
                    <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1"><Phone size={11} /> Téléphone</div>
                    <p className="text-white font-semibold text-xs">{ticket.holder_phone}</p>
                  </div>
                )}
                <div>
                  <div className="text-white/40 text-xs mb-1">Montant</div>
              ...
              {ticket.qr_code && (
                <div className="flex flex-col items-center py-4">
                  ...
                  <p className="text-white/30 text-xs mt-3 font-mono tracking-wider">{ticket.ticket_uuid.toUpperCase().slice(0, 20)}</p>
                  <p className="text-white/20 text-xs mt-1">ADMIT ONE · VALIDE 1 PERSONNE</p>
                </div>
              )}
            </div>
          </div>
